...
for($y=$YBITS-$y_pp_size; $y < $YBITS ; $y++) {
    for($x=$XBITS-2; $x >= $XBITS-$x_pp_size; $x--) {
        if($x+$y>=$XBITS+$YBITS-$ZBITS){
            if($y==$YBITS-$y_pp_size+1){
	        printf("\thalfNAdder  HA%s(carry%s[%s],sum%s[%s],P%s[%s],P%s[%s]);\n",
		       $ha_label,$y,$x,$y,$x,$y,$x,$y-1,$x+1);
	        $ha_label++;
	    }
	    if($y>$YBITS-$y_pp_size+1){
                if($x+$y==$XBITS+$YBITS-$ZBITS){
	            printf("\thalfNAdder  HA%s(carry%s[%s],sum%s[%s],P%s[%s],sum%s[%s]);\n",
			   $ha_label,$y,$x,$y,$x,$y,$x,$y-1,$x+1);
	            $ha_label++;
		}
	        else{
	            printf("\tfullNAdder  FA%s(carry%s[%s],sum%s[%s],P%s[%s],sum%s[%s],carry%s[%s]);\n",
...
for($x=$xstart; $x < $XBITS ; $x++) {
    if($x==$xstart) {
	if($x==$XBITS-$nhop){
	    printf("\thalfAdder CPA%s(carry%s[%s],Z[%s],carry%s[%s],sum%s[%s]);\n",$cpa_label,$YBITS,$x,$Zpin,$YBITS-1,$x,$YBITS-1,$x+1);
	    $cpa_label++;
	    $Zpin++;
	}
	else{
